...
class BackpackAPI {
   ...
   * }>>}
   */
  async getSpotKlines(symbol, interval = '1m', limit = 1000) {
    const params = {
      symbol,
      interval,
      limit,
  ...
  async getKlines(params) {
    ...
    const endTime = Math.floor(Date.now() / 1000); // 当前时间戳（秒）
    const startTime = endTime - (limit * this.intervalMs[interval] || 3600); // 默认1小时

    return await this.api('klines', {
      symbol,
      interval,
      limit,
      startTime, // 使用秒级时间戳
