...
function loadConfigFromFile(configFile, strategyName) {
    try {
        ...
        if (!fs.existsSync(configFile)) {
            ...
            console.log('💡 提示: 请先复制 backpack-grid-config-template.json 为 backpack-grid-config.json 并填入你的配置');
            process.exit(1);
        }
    ...
    try {
        ...
        if (config.strategies) {
            Object.entries(config.strategies).forEach(([key, strategy]) => {
                ...
                console.log(`   每格数量: ${strategy.amountPerGrid}`);
                console.log(`   订单类型: ${strategy.orderType || 'Limit'}`);
            });
        }
        
        console.log('\n' + '='.repeat(70));
        console.log('\n使用方法:');
        console.log(`  node backpack-grid-runner.js <策略名称>`);
        console.log(`  例如: node backpack-grid-runner.js sol_conservative\n`);
...
async function runStrategy(config) {
    ...
    console.log(`   订单类型: ${config.orderType || 'Limit'}`);
...
function setupGracefulShutdown(strategy) {
    const shutdown = () => {
        console.log('\n\n⏸️  收到退出信号，正在停止策略...');
        strategy.stop();
        console.log('👋 感谢使用Backpack网格交易策略！');
        process.exit(0);
    };
...
async function main() {
    ...
    const configFile = './backpack-grid-config.json';
    ...
    if (args.length === 0) {
        ...
        console.log('使用方法:');
        console.log('  node backpack-grid-runner.js <策略名称>');
        console.log('  node backpack-grid-runner.js list  # 列出所有可用策略\n');
    ...
    if (!config.apiKey || config.apiKey === 'your_api_key_here' || config.apiKey === undefined) {
        console.error('\n❌ 错误: API密钥未配置');
        console.log('💡 提示: 请在 backpack-grid-config.json 中配置 API 密钥');
        process.exit(1);
    }
    
    if (!config.apiSecret || config.apiSecret === 'your_api_secret_here' || config.apiSecret === undefined) {
        console.error('\n❌ 错误: API Secret未配置');
        console.log('💡 提示: 请在 backpack-grid-config.json 中配置 API Secret');
        process.exit(1);
    }
